...
def interpolate_outliers(ts, cut=5.5):
    ...
    while any(abs(zscore) > cut):
        ...
        ts = np.delete(ts, outliers)
        for idx in outliers[0]:
            nremoved += 1

            """
            if idx > 5 and idx < len(ts)-6:
                ts[idx] = sum(ts[idx-5:idx+5]) / len(ts[idx-5:idx+5])
            elif idx == 0:
                ts[idx] = ts[1]
            elif idx > 0:
                ts[idx] = ts[idx-1]
...
def load_as_df(fname, ref=False, filt=True, srate=1024):
    ...
    df = pd.DataFrame(raw.get_data(), raw.ch_names).transpose()
    data = df[df.columns[0]].values
    data = data[10*srate:90*srate]
    data, nremoved = interpolate_outliers(data)

    #bandpass filter
    data = mne.filter.filter_data(data, srate, l_freq=5, h_freq=None)

    #bandstop filter
    data = mne.filter.notch_filter(data, srate, 60, trans_bandwidth=2)
