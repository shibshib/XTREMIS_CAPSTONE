...
void stopButtonWasPressed() {
  ...
  if (isRunning) {
    ...
    if (eegDataSource == DATASOURCE_XTREMIS && xt.isCheckingImpedance()) {
      xt.impedanceStop();
...
class OpenBCI_ADS1299 {
  ...
  private final float fs_Hz = 1024.0f;  //sample rate used by XTREMIS board...
  ...
  public void stopDataTransfer() {
      xt.changeState(STATE_STOPPED);  // make sure it's now interpretting as binary
      println("OpenBCI_ADS1299: startDataTransfer(): writing \'" + command_stop + "\' to the serial port...");
   //   xt.write(command_stop);// + "\n");
