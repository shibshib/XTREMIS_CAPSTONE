...
int getDataIfAvailable(int pointCounter) {
  ...
  if (eegDataSource == DATASOURCE_XTREMIS) {
    ...
    while ( (curDataPacketInd != lastReadDataPacketInd) && (pointCounter < nPointsPerUpdate)) {
     ...
     //   yLittleBuff_uV[Ichan][pointCounter] = dataPacketBuff[lastReadDataPacketInd].values[Ichan] * ganglion.get_scale_fac_uVolts_per_count();
        yLittleBuff_uV[Ichan][pointCounter] = dataPacketBuff[lastReadDataPacketInd].values[Ichan] * xt.get_scale_fac_uVolts_per_count();
      }
      pointCounter++; //increment counter for "little buffer"
    }
...
class DataProcessing {
  ...
  final int N_SNOTCH_CONFIGS = 3;
  FilterConstants[] filtCoeff_sNotch = new FilterConstants[N_SNOTCH_CONFIGS];
  private int currentFilt_ind = 3;
  private int currentNotch_ind = 0;  // set to 0 to default to 60Hz, set to 1 to default to 50Hz
  private int currentsNotch_ind = 0;
  float data_std_uV[];
  float polarity[];
  ...
  //define filters...assumes sample rate of 1024 Hz !!!!!
  private void defineFilters(int _mode) {
    ...
    case 0:
      ...
      for (int Ifilt=0; Ifilt < n_filt; Ifilt++) {
        switch (Ifilt) {
        case 0:
          //60 Hz notch filter, assumed fs = 1024 Hz.  4th Order Butterworth: b, a = signal.butter(4,[59.0 61.0]/(fs_Hz / 2.0), 'bandstop')
          b2 = new double[] {0.9889542480671399, -3.5247674607365505,  5.118596166102134 ,
       -3.5247674607365496,  0.9889542480671395};
          a2 = new double[] { 1.                , -3.5443430247599608,  5.1184741537446197,
       -3.5051918967131415,  0.9780305084917962};
          // OLD COEFFICIENTS FOR 250 HZ
          ...
          break;
        case 1:
          //50 Hz notch filter, assumed fs = 1024 Hz.  4th Order Butterworth: b, a = signal.butter(4,[49.0 51.0]/(fs_Hz / 2.0), 'bandstop')
          b2 = new double[] { 0.9889542480671395, -3.6548110764987283,  5.3546177905744994,
       -3.6548110764987292,  0.9889542480671399};
          a2 = new double[] {1.                , -3.6751088660744751,  5.3544957782169833,
       -3.6345132869229819,  0.9780305084917956};
          // OLD COEFFICIENTS FOR 250 Hz
      ...
      for (int Ifilt=0; Ifilt<n_filt; Ifilt++) {
        ...
        switch (Ifilt) {
        case 0:
          b = new double[] {0.0288999526499366,  0.                , -0.0577999052998733,   0.                ,  0.0288999526499366};
          a = new double[] { 1.                , -3.4594457398272449,  4.5006630944481447, -2.6216642700481634,  0.5804543364015973};
         /* OLD COEFFICIENTS
          ...
          break;
        case 1:
          b = new double[] {0.0005371697748121,  0.                , -0.0010743395496241, 0.                ,  0.0005371697748121};
          a = new double[] {1.                , -3.9223436514150789,  5.7806084631919159, -3.7937632570810282,  0.9355289049791782};
        // OLD COEFFICIENTS FOR 250 SPS
        /*
          //butter(2,[7 13]/(250/2));
          b = new double[] {
          ...
          break;
        case 2:
           b = new double[] {0.015749469765994,  0.               , -0.031498939531988, 0.               ,  0.015749469765994};
           a = new double[] {1.                , -3.5311159447514262,  4.7568794879065583, -2.9019333652131793,  0.6779404932514446};
        // OLD COEFFICIENTS FOR 250 SPS
         /*
          ...
          break;
        case 3:
            b = new double[] { 0.0248279073387552,  0.                , -0.0496558146775105, 0.                ,  0.0248279073387552};
           a = new double[] {1.                , -3.4801528787893763,  4.5733736858953398, -2.6997912516918339,  0.60675903349847 };
          // OLD COEFFICIENTS FOR 250 SPS
      ...
      n_filt = filtCoeff_sNotch.length;
      for (int Ifilt=0; Ifilt<n_filt; Ifilt++) {
        //define bandpass filter
        switch (Ifilt) {
          case 0:        
            ...
            b2 = new double[] { 1.0 };
            a2 = new double[] { 1.0 };
            filtCoeff_sNotch[Ifilt] =  new FilterConstants(b2, a2, "No Sec Notch", "None");
              break;
          case 1:        // Filter out 30Hz
            //30 Hz notch filter, assumed fs = 1024 Hz.  4th Order Butterworth: b, a = signal.butter(4,[15.0 17.0]/(fs_Hz / 2.0), 'bandstop')
              b2 = new double[] {   0.9870680830317123, -3.8839316195905265,  5.7947755639362581,
       -3.8839316195905269,  0.9870680830317123};
              a2 = new double[] { 1.                , -3.9092094625571403,  5.7946083224673552,
       -3.8586537766239131,  0.9743034075323267};
              // OLD COEFFICIENTS FOR 250 HZ
              //b2 = new double[] { 9.650809863447347e-001, -2.424683201757643e-001, 1.945391494128786e+000, -2.424683201757643e-001, 9.650809863447347e-001 };
              //a2 = new double[] { 1.000000000000000e+000, -2.467782611297853e-001, 1.944171784691352e+000, -2.381583792217435e-001, 9.313816821269039e-001  };
              filtCoeff_sNotch[Ifilt] =  new FilterConstants(b2, a2, "Sec Notch 30Hz", "30Hz");
              break;
          case 2:
            //16 Hz notch filter, assumed fs = 1024 Hz.  4th Order Butterworth: b, a = signal.butter(4,[29.0 31.0]/(fs_Hz / 2.0), 'bandstop')
              b2 = new double[] { 0.9870680830317122, -3.9282209171389368,  5.8824074999041596,
       -3.9282209171389377,  0.9870680830317125};
              a2 = new double[] {1.                , -3.9537870087201461,  5.8822402584352584,
       -3.9026548255577289,  0.9743034075323269};
              // OLD COEFFICIENTS FOR 250 HZ
              //b2 = new double[] { 9.650809863447347e-001, -2.424683201757643e-001, 1.945391494128786e+000, -2.424683201757643e-001, 9.650809863447347e-001 };
              //a2 = new double[] { 1.000000000000000e+000, -2.467782611297853e-001, 1.944171784691352e+000, -2.381583792217435e-001, 9.313816821269039e-001  };
              filtCoeff_sNotch[Ifilt] =  new FilterConstants(b2, a2, "Sec Notch 16Hz", "16Hz");
            
            break;
        }
      } // end secondary notch filters
  ...
  public String getFilterDescription() {
    return filtCoeff_bp[currentFilt_ind].name + ", " + filtCoeff_notch[currentNotch_ind].name + ", " + filtCoeff_sNotch[currentsNotch_ind].name;
  }
  ...
  public String getShortSecondaryNotchDescription() {
    return filtCoeff_sNotch[currentsNotch_ind].short_name;
  }
  ...
  public void incrementSecondaryNotchConfiguration() {
    currentsNotch_ind++;
    if(currentsNotch_ind >= N_SNOTCH_CONFIGS) currentsNotch_ind = 0;
  }
  public void process(float[][] data_newest_uV, //holds raw EEG data that is new since the last call
    ...
    for (int Ichan=0; Ichan < nchan; Ichan++) {
      ...
      filterIIR(filtCoeff_sNotch[currentsNotch_ind].b, filtCoeff_sNotch[currentsNotch_ind].a, data_forDisplay_uV[Ichan]); // secondary notch
