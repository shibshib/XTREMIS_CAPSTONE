...
final int DATASOURCE_XTREMIS = 0; // new default, data from XT
...
boolean isXtremisObjectInitialized = false;
color bgColor = color(0, 0, 0);//color(1, 18, 41);
...
void setup() {
  ...
  println("Welcome to the Processing-based XTREMIS GUI!"); //Welcome line.
  println("Last update: July 2017"); //Welcome line.
  ...
  frame.addComponentListener(new ComponentAdapter() {
    public void componentResized(ComponentEvent e) {
      if (e.getSource()==frame) {
        println("XTREMIS_GUI: setup: RESIZED");
  ...
  cog = loadImage("startup_logo.PNG");
  ...
  try {
    ...
    verbosePrint("X.pde: could not open " + serial_output_portName);
  }
...
void initSystem() {
  ...
  verbosePrint("XTREMIS_GUI: initSystem: -- Init 0 -- " + millis());
  timeOfInit = millis(); //store this for timeout in case init takes too long
  verbosePrint("timeOfInit = " + timeOfInit);

  //prepare data variables
  verbosePrint("XTRMS_GUI: initSystem: Preparing data variables...");
  ...
  verbosePrint("XTRMS_GUI: initSystem: -- Init 1 -- " + millis());
  ...
  verbosePrint("XTREMIS_GUI: initSystem: -- Init 2 -- " + millis());

  //prepare the source of the input data
  switch (eegDataSource) {
  case DATASOURCE_XTREMIS:
    int nEEDataValuesPerPacket = nchan;
    boolean useAux = false;
    // DO NOT Enable triggers by default.. Instead, add button to enable triggers.
    // if (eegDataSource == DATASOURCE_XTREMIS) useAux = true;  //switch this back to true CHIP 2014-11-04
    xt.connectWIFI(xtremis_portName);
  ...
  verbosePrint("XTREMIS_GUI: initSystem: -- Init 3 -- " + millis());
  ...
  if (abandonInit) {
    ...
    if (!abandonInit) {
      ...
      if (eegDataSource == DATASOURCE_XTREMIS){ 
        openNewLogFile(fileName);  //open a new log file
      }
  ...
  verbosePrint("XTREMIS_GUI: initSystem: -- Init 4 -- " + millis());
...
void systemUpdate() { // for updating data values and variables
   ...
   */
  if (isXtremisObjectInitialized == false && millis() - timeOfSetup >= 1500) {
    xt = new XTREMIS(this);
    println("Instantiating XTREMIS object...");
    isXtremisObjectInitialized = true;
  }
...
void systemDraw() { //for drawing to the screen
  ...
  if (systemMode >= SYSTEMMODE_POSTINIT) {
    ...
    surface.setTitle(int(frameRate) + " fps — XTREMIS GUI");
  }
