...
class W_accelerometer extends Widget {
  ...
  void update(){
    ...
    if (isRunning) {
      if (eegDataSource == DATASOURCE_SYNTHETIC) {
        ...
        currentXvalue = xt.accelArray[0] * xt.get_scale_fac_accel_G_per_count();
        currentYvalue = xt.accelArray[1] * xt.get_scale_fac_accel_G_per_count();
        currentZvalue = xt.accelArray[2] * xt.get_scale_fac_accel_G_per_count();
        X[X.length-1] =
  ...
  void draw(){
    ...
    if (true) {
      ...
      if (eegDataSource == DATASOURCE_XTREMIS) {  // LIVE
        // fill(Xcolor);
