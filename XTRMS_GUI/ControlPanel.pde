...
//    Edited by: Ala Shaabana (Jul. 2017)
//
//////////////////////////////////////////////////////////////////////////
...
public void controlEvent(ControlEvent theEvent) {
  ...
  if (theEvent.isFrom("sourceList")) {
    ...
    if(newDataSource == DATASOURCE_XTREMIS){
      updateToNChan(8);
...
class ControlPanel {
  ...
  boolean showSourceBox, showWifiBox, showFileBox, showChannelBox, showInitBox;
  ...
  ControlPanel(XTRMS_GUI mainClass) {
    ...
    dataLogBox = new DataLogBox(x + w, (WifiBox.y + WifiBox.h), w, h, globalPadding);
  ...
  public void draw() {
    ...
    if (systemMode != 10) { // only draw control panel boxes if system running is false
      ...
      if (eegDataSource == DATASOURCE_XTREMIS) {	//when data source is from OpenBCI
        // hideAllBoxes();
        WifiBox.draw();
      ...
      //      cp5Popup.get(MenuList.class, "pollList").setVisible(false);
            cp5.get(MenuList.class, "serialList").setVisible(false); //make sure the serialList menulist is visible
  ...
  public void CPmousePressed() {
    ...
    if (systemMode != 10) {
      ...
      //active buttons during DATASOURCE_XTREMIS
...
*/
      ...
      //active buttons during ble devices
...
public void initButtonPressed(){
  if (initSystemButton.but_txt == "START SYSTEM") {
      ...
      if (eegDataSource == DATASOURCE_XTREMIS && !xt.getStatus()) { //if data source == normal && if no serial port selected OR no SD setting selected
        output("NOTE: Make sure you are connecting to Wisergroup WiFi network.");
        initSystemButton.wasPressed = false;
        initSystemButton.setIsActive(false);
        return;
      ...
      //    verbosePrint("ControlPanel — port is open: " + ganglion.isPortOpen());
        ...
        } else if(eegDataSource == DATASOURCE_XTREMIS){
...
class DataSourceBox {
  ...
  //  sourceList.addItem(makeItem("LIVE (from Cyton)"));
    sourceList.addItem(makeItem("LIVE XTREMIS"));
